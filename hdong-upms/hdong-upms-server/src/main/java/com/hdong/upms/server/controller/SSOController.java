...
import com.hdong.upms.server.constant.UpmsResult;
import com.hdong.upms.server.constant.UpmsResultConstant;
...
public class SSOController extends BaseController {
  ...
  public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
    ...
    // // 全局会话的code
    // RedisUtil.set(HDONG_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int)
    // subject.getSession().getTimeout() / 1000);
    // // code校验值
    // RedisUtil.set(HDONG_UPMS_SERVER_CODE + "_" + code, code, (int)
    // subject.getSession().getTimeout() / 1000);
    // }
    // 回跳登录前地址
    String backurl = request.getParameter("backurl");
    RootController.hasSession = true;
    if (StringUtils.isBlank(backurl)) {
